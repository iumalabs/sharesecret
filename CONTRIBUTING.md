...
- **Build & tooling:** [Deno](https://deno.com/) — `deno.json` is the only manifest (no `package.json`/`package-lock.json`). All dependencies are npm packages, resolved via Deno's `npm:` specifiers and `deno.lock`; every `deno task` below just runs the same underlying tool (Vite, Wrangler, Vitest, Playwright, ESLint, Prettier, tsc) through Deno instead of Node. `nodeModulesDir: "auto"` in `deno.json` still materializes a local `node_modules/` (gitignored) since these tools assume Node-style resolution -- Deno itself only replaces the package manager and task runner here, not the tools.
...
| `deno task typecheck`               | `tsc --noEmit` for both the Worker and the React app                            |
